...
const createOrder = async (userId, totalPrice, items, options = {}) => {
  ...
  try {
    ...
    const order = await createOrderWithClient(client, userId, totalPrice, items, options);
...
const getOrdersByUser = (userId) =>
  db.query(
    `SELECT o.*,
            ...
            COALESCE(history.status_history, '[]'::json) AS status_history
     FROM orders o
     LEFT JOIN LATERAL (
       SELECT json_agg(
         json_build_object(
...
const updateStatus = async (orderId, { status, changedBy, note, fulfillment = {} }) => {
  const client = await db.pool.connect();
  try {
    await client.query('BEGIN');
    const { rows: currentRows } = await client.query('SELECT * FROM orders WHERE id = $1 FOR UPDATE', [orderId]);
    ...
    const updateResult = await client.query(
      ...
      [
        nextStatus,
        fulfillment.courier_name || null,
        fulfillment.tracking_number || null,
        fulfillment.estimated_delivery_date || null,
        fulfillment.admin_notes || null,
        orderId,
      ]
    );
    ...
    if (current.status !== nextStatus) {
      await client.query(
        `INSERT INTO order_status_history (order_id, from_status, to_status, changed_by, note)
         VALUES ($1, $2, $3, $4, $5)`,
        [orderId, current.status, nextStatus, changedBy || null, note || null]
      );
    }
...
module.exports = { createOrder, createOrderWithClient, getOrdersByUser, getCustomerOrdersForAdmin, getAllOrders, getOrderById, updateStatus, updatePaymentStatus, sumRevenue, countOrders, hasDeliveredProduct };
